...
generare shell command arbitrari.
...
workspace-scoped, cancellabile e non persistente.
...
Jarvis non sceglie casualmente: un pareggio produce una domanda breve.
...
persiste il tail e non usa automaticamente il codice sorgente. Il Context
Broker può fornire documentazione Markdown consentita, summary workspace,
registry, task e risultati bounded.

## PTY visibile e catalogo agenti

Il catalogo typed runtime supporta soltanto i provider conosciuti dal progetto:
Codex, OpenCode, Pi, Freebuff e Claude Code. Non esiste network discovery e non
è previsto provider fallback silenzioso.
...
dimostrata pronta. Il frontend registra la sessione tramite l'evento backend,
così non viene creato un processo hidden o una seconda PTY. Non vengono usati
...
sintetizza un prompt compatto e autosufficiente entro 6 KiB di contesto
derivato. Non trasferisce thinking o raw output completo e non scansiona
automaticamente il codice sorgente. Il destinatario può usare i propri tool
nella workspace.
...
I test backend coprono piani typed, isolamento workspace, risoluzione chiara e
ambigua, busy clarification, draft-only, generation mismatch, tail bounded,
spawn/readiness, handoff, failure stop, no fallback e conferme distruttive
dialogiche. I test frontend coprono la rimozione della card per i send normali,
clarification conversazionale, voice flow e assenza di notifiche autonome.
