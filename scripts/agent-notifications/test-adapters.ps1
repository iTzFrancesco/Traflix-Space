...
$uiContracts = @(
    ...
    @{ Name = "main notification receiver"; Path = Join-Path $scriptRoot "..\..\src\components\agent\AgentCompletionListener.tsx"; Required = @("[agent-notification] open requested", "const mainWindow = getCurrentWebviewWindow()", "await mainWindow.unminimize()", "await mainWindow.show()", "await mainWindow.setFocus()", "[agent-notification] main window opened"); Forbidden = @('WebviewWindow.getByLabel("main")', "terminal_write", "terminal_kill", "create_workspace", "update_workspace", "delete_workspace") },
    ...
    @{ Name = "overlay native window"; Path = Join-Path $scriptRoot "..\..\src-tauri\tauri.conf.json"; Required = @('"label": "agent-notification"', '"url": "index.html?agent-notification"', '"alwaysOnTop": true', '"skipTaskbar": true', '"visible": false') }
)
