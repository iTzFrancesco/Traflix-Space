...
- Available terminal-agent providers are: `anti-gravity`/`agy` (Anti-Gravity), `pi` (pi.dev; also "p" or "agente P"), `codex` (OpenAI Codex CLI), `opencode` (OpenCode), `claude` (Claude Code), `claudex` (Claudex), `cmdc`, `cline`, `freebuff`, and `grok` (Grok CLI).
