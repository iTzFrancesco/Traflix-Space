...
export function JarvisTranscriptCard({ request, activeWorkspace, onSend, onDiscard }: { request: VoiceRequestStatusView; activeWorkspace: boolean; onSend: (text: string) => Promise<void> | void; onDiscard: () => void }) {
  ...
  return <section className="mt-3 rounded-xl border border-signal/30 bg-signal/[0.06] p-3" data-jarvis-control>
    ...
    <div className="mt-2 flex gap-2"><button type="button" disabled={!activeWorkspace || !text.trim() || sending} onClick={async () => { setSending(true); try { await onSend(text); } finally { setSending(false); } }} className="inline-flex items-center gap-1 rounded-md bg-signal px-2 py-1 text-xs font-semibold text-neutral-bg disabled:opacity-40"><Check size={13} /> {sending ? "Invio…" : "Invia alla chat"}</button><button type="button" disabled={sending} onClick={onDiscard} className="inline-flex items-center gap-1 rounded-md border border-white/10 px-2 py-1 text-xs text-neutral-text-muted disabled:opacity-40"><Trash2 size={13} /> Scarta</button></div>
  </section>;
}
